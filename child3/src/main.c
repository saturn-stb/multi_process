...
static void* recv_thread_func(void* arg)
{
	...
	while (1) 
	{
		...
		if (shm_Queue != NULL) 
		{
			...
			{
				printf("\n[CHILD3-RECV] Queue Full! Dropping.\n");
				sem_post(sem_mutex);
				continue;
			}
			...
			{
				printf("\n[CHILD3-RECV] ID mismatch!!! (0x%02x)\n", proc_id);
			...
			printf("\n[CHILD3-RECV] Message from PARENT : %s\n", msg_data);
...
static void* send_thread_func(void* arg)
{
	...
	while (1)
	{
        ...
        if (shm_Queue != NULL) 
        {
	        ...
	        {
	            printf("\n[CHILD3-SEND] Queue Full, waiting...\n");
	            sem_post(sem_mutex);
	            usleep(50000);
	            continue;
	        }
			...
			printf("\n[CHILD3-SEND] Message to CHILD2 : %s\n", msg_data);
#else
            int pos = shm_Queue->head;
	        ...
	        printf("\n[CHILD3-SEND] Message to PARENT : %s\n", msg_data);
#endif
	        work_done = 0; // 플래그 리셋
	        sem_post(sem_mutex);
